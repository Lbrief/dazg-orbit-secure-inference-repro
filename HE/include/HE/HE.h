...
class HEEvaluator {
    ...
    void ReceiveEncVec(Tensor<unified::UnifiedCiphertext> &ct_vec){
        ...
        if (vec_size != ct_vec.size()) {
            std::cerr << "[DAZG_ORBIT_V560_RECV_ENC_VEC_SIZE_MISMATCH]"
                      << " marker=DAZG_ORBIT_V560_SENDRECV_DIAGNOSTICS_20260613"
                      << " expected=" << ct_vec.size()
                      << " received=" << vec_size
                      << " action=assert_protocol_mismatch"
                      << std::endl;
        }
