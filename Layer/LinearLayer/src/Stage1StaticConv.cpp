...
// Component: Layer/LinearLayer/src/Stage1StaticConv.cpp
...
// 计算上取整除法
int Conv2DStage1Static::DivUpper(int a, int b) {
    return ((a + b - 1) / b);
}

// 计算计算开销
int Conv2DStage1Static::CalculateCost(int H, int W, int h, int Hw, int Ww, int C, int N) {
...
// 查找最佳分块方式
void Conv2DStage1Static::FindOptimalPartition(int H, int W, int h, int C, int N, int* optimalHw, int* optimalWw) {
    int min_cost = (1 << 30);
...
void Conv2DStage1Static::compute_he_params(uint64_t raw_in_feature_size) {
...
Conv2DStage1Static::Conv2DStage1Static(uint64_t in_feature_size,
                                           uint64_t stride,
                                           uint64_t padding,
    ...
    : Conv2D(in_feature_size, stride, padding, weight, bias, HE)
{
    ...
    const bool hot_stage1 =
        (in_channels == 64 && out_channels == 64 &&
         kernel_size == 3 && stride == 1 && HOut == 56 && WOut == 56);

#ifdef PROFILE_STAGE1_STATIC
    if (hot_stage1) {
        std::cout << "[Stage1StaticCtor] "
...
Conv2DStage1Static::Conv2DStage1Static(
...
void Conv2DStage1Static::PreparePlan() {
...
void Conv2DStage1Static::fuse_bn(Tensor<uint64_t> *gamma, Tensor<uint64_t> *beta){
...
Conv2DStage1Static::Conv2DStage1Static(
    ...
    : Conv2DStage1Static(in_feature_size, stride, padding, weight, bias, HE)
{
    this->fused_bn = true;
...
// 加密张量
Tensor<UnifiedCiphertext> Conv2DStage1Static::EncryptTensor(Tensor<UnifiedPlaintext> plainTensor) {
...
Tensor<uint64_t> Conv2DStage1Static::HETOTensor (Tensor<UnifiedCiphertext> inputCipher){
    ...
    if (HE->server) {
        int64_t mask;
        auto gen = dazg_orbit_det::MakeMt19937("Stage1StaticConv");
...
// 计算输入张量的 Pack 版本
Tensor<uint64_t> Conv2DStage1Static::PackActivation(Tensor<uint64_t> &x){
...
Tensor<UnifiedCiphertext> Conv2DStage1Static::TensorTOHE(Tensor<uint64_t> PackActivationTensor) {
...
// 计算卷积核的 Pack 版本
Tensor<UnifiedPlaintext> Conv2DStage1Static::PackWeight() {
...
Tensor<UnifiedCiphertext> Conv2DStage1Static::sumCP(Tensor<UnifiedCiphertext> cipherTensor, Tensor<UnifiedPlaintext> plainTensor){
...
Tensor<UnifiedCiphertext> Conv2DStage1Static::HECompute(
...
Tensor<uint64_t> Conv2DStage1Static::DepackResult(Tensor<uint64_t> &out){
    if (!plan_ready) {
        PreparePlan();
    }
...
Tensor<uint64_t> Conv2DStage1Static::operator()(Tensor<uint64_t> &x){
    long long plan_us = 0;
    if (!plan_ready) {
        auto tp0 = Clock::now();
        PreparePlan();
...
void Conv2DStage1Static::FillActivationPlaintexts(
    Tensor<uint64_t> &x,
...
Tensor<UnifiedCiphertext> Conv2DStage1Static::PackActivationDirectToHE(Tensor<uint64_t> &x) {
    if (!plan_ready) {
        PreparePlan();
    }
    ...
    dazg_orbit::domain::ScopedConversionRecord dazg_orbit_domain_record(
        ...
        "Conv2DStage1Static::PackActivationDirectToHE");
