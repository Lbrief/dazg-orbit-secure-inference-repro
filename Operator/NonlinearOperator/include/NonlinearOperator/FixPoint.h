...
// Component: Operator/NonlinearOperator/include/NonlinearOperator/FixPoint.h
...
class FixPoint {
    public:
        int DAZGOrbitActiveThreadCountV40(int dim) const {
            if (dim <= 0) return 0;
            if (num_threads <= 1) return 1;
            return std::min(num_threads, dim);
        }

        static int DAZGOrbitChunkBeginV40(int dim, int tid, int active_threads) {
            return static_cast<int>((static_cast<long long>(dim) * tid) / active_threads);
        }

        static int DAZGOrbitChunkEndV40(int dim, int tid, int active_threads) {
            return static_cast<int>((static_cast<long long>(dim) * (tid + 1)) / active_threads);
        }
        ...
        void less_than_zero(Tensor<T> &x, Tensor<uint8_t> &result, int32_t bw){
            ...
            std::vector<std::thread> less_than_threads;
            const int active_threads = DAZGOrbitActiveThreadCountV40(dim);
            less_than_threads.reserve(active_threads);
            for (int i = 0; i < active_threads; i++) {
                const int begin = DAZGOrbitChunkBeginV40(dim, i, active_threads);
                const int end = DAZGOrbitChunkEndV40(dim, i, active_threads);
                if (begin >= end) continue;
                less_than_threads.emplace_back(less_than_thread, aux[i],
                    x_flatten + begin, result_flatten + begin, end - begin, bw);
            }
            for (auto& th : less_than_threads) {
                th.join();
            }
        }
        ...
        // for now, only support 64-bit ring tensors. TODO: support other types
        // DAZG_ORBIT_V526_SIGNED_TRUNCATE_FIX:
        // ResNet/GeLU callers pass the fourth argument as signed_trunc=true.
        // The old implementation interpreted that bool as msb_zero and still used
        // unsigned truncation for uint64_t shares, which corrupts negative fixed-point
        // values before DAZG/GeLU and before the final classifier rescale.
        void truncate(Tensor<T> &x, int32_t shift, int32_t bw, bool signed_arithmetic=false){
            auto shape = x.shape();
            int dim = x.size();
            x.flatten();
            T* x_flatten = x.data().data();
            std::vector<std::thread> truncation_threads;
            const bool use_signed_arithmetic = signed_arithmetic || std::is_signed_v<T>;
            uint8_t* msb_slice = nullptr;
            const int active_threads = DAZGOrbitActiveThreadCountV40(dim);
            truncation_threads.reserve(active_threads);
            for (int i = 0; i < active_threads; i++) {
                const int begin = DAZGOrbitChunkBeginV40(dim, i, active_threads);
                const int end = DAZGOrbitChunkEndV40(dim, i, active_threads);
                if (begin >= end) continue;
                truncation_threads.emplace_back(truncation_thread, truncationProtocol[i],
                    x_flatten + begin, x_flatten + begin, end - begin,
                    shift, bw, use_signed_arithmetic, msb_slice);
            }
            for (auto& th : truncation_threads) {
                th.join();
            }
            x.reshape(shape);
        }
        ...
        void extend(Tensor<T> &x, int32_t bwA, int32_t bwB, bool msb_zero=false){
            ...
            const int active_threads = DAZGOrbitActiveThreadCountV40(dim);
            extend_threads.reserve(active_threads);
            const bool signed_arithmetic = std::is_signed_v<T>;
            for (int i = 0; i < active_threads; i++) {
                const int begin = DAZGOrbitChunkBeginV40(dim, i, active_threads);
                const int end = DAZGOrbitChunkEndV40(dim, i, active_threads);
                if (begin >= end) continue;
                extend_threads.emplace_back(extend_thread, aux[i],
                    x_flatten + begin, x_flatten + begin, end - begin,
                    bwA, bwB, signed_arithmetic, msb_zero);
            }
            for (auto& th : extend_threads) {
                th.join();
            }
        }
        ...
        // DAZG_ORBIT_V543_RING2FIELD_SIGNED_GUARD
        void Ring2Field(Tensor<T> &x, ModulusType Q, int bitwidth = 0){
            if (bitwidth == 0){
                bitwidth = x.bitwidth;
            }
            // DAZG_ORBIT_V531_RING2FIELD_SIGNED_U64_FIX:
            // uint64_t tensors store signed fixed-point ring shares.  The old path
            // used zero-extension for uint64_t, so negative ring values were lifted as
            // large positives before conversion to the BFV field.
            int ext_bit = 60 - bitwidth;
            if (ext_bit < 0) ext_bit = 0;
            int total_bw = bitwidth + ext_bit;
            {
                ...
                const int active_threads = DAZGOrbitActiveThreadCountV40(dim);
                extend_threads.reserve(active_threads);
                const bool signed_arithmetic = true;
                for (int i = 0; i < active_threads; i++) {
                    const int begin = DAZGOrbitChunkBeginV40(dim, i, active_threads);
                    const int end = DAZGOrbitChunkEndV40(dim, i, active_threads);
                    if (begin >= end) continue;
                    extend_threads.emplace_back(extend_thread, aux[i],
                        x_flatten + begin, x_flatten + begin, end - begin,
                        bitwidth, total_bw, signed_arithmetic, false);
                }
                for (auto& th : extend_threads) {
                    th.join();
                }
        ...
        void Field2Ring(Tensor<T> &x, ModulusType Q, int bitwidth = 0){
            ...
            if constexpr (sizeof(T) == sizeof(uint64_t)) {
                std::vector<std::thread> field2ring_threads;
                field2ring_threads.reserve(active_threads);
                for (int i = 0; i < active_threads; i++) {
                    const int begin = DAZGOrbitChunkBeginV40(dim, i, active_threads);
                    const int end = DAZGOrbitChunkEndV40(dim, i, active_threads);
                    if (begin >= end) continue;
                    field2ring_threads.emplace_back(field2ring_thread, aux[i],
                        x.data().data()+begin, x.data().data()+begin,
                        end-begin, bitwidth, Q, signed_arithmetic);
                }
                for (auto& th : field2ring_threads) {
                    th.join();
                }
                ...
                for (int i = 0; i < active_threads; i++) {
                    ...
                    const int end = DAZGOrbitChunkEndV40(dim, i, active_threads);
                    if (begin >= end) continue;
                    field2ring_threads.emplace_back(field2ring_thread_128, aux[i],
                        x.data().data()+begin, x.data().data()+begin,
                        end-begin, bitwidth, Q, signed_arithmetic);
                }
                for (auto& th : field2ring_threads) {
        ...
        void mux(Tensor<uint8_t> &b, Tensor<T> &input, Tensor<T> &result, int32_t bwA, int32_t bwB){
            ...
            std::vector<std::thread> mux_threads;
            const int active_threads = DAZGOrbitActiveThreadCountV40(dim);
            mux_threads.reserve(active_threads);
            for (int i = 0; i < active_threads; i++) {
                const int begin = DAZGOrbitChunkBeginV40(dim, i, active_threads);
                const int end = DAZGOrbitChunkEndV40(dim, i, active_threads);
                if (begin >= end) continue;
                mux_threads.emplace_back(mux_thread, aux[i],
                    b.data().data() + begin, input_flatten + begin, result_flatten + begin,
                    end - begin, bwA, bwB);
            }
            for (auto& th : mux_threads) {
                th.join();
            }
        }
        ...
        void secure_requant(Tensor<T> &x, double scale_in, double scale_out, 
            ...
            std::vector<std::thread> requant_threads;
            const int active_threads = DAZGOrbitActiveThreadCountV40(dim);
            requant_threads.reserve(active_threads);
            for (int i = 0; i < active_threads; i++) {
                const int begin = DAZGOrbitChunkBeginV40(dim, i, active_threads);
                const int end = DAZGOrbitChunkEndV40(dim, i, active_threads);
                if (begin >= end) continue;
                requant_threads.emplace_back(secure_requant_thread, aux[i],
                    x_flatten + begin, x_flatten + begin,
                    end - begin, scale_in, scale_out,
                    bw_in, bw_out, s_fix, party);
            }
            for (auto& th : requant_threads) {
                th.join();
            }
            x.reshape(shape);
        }
