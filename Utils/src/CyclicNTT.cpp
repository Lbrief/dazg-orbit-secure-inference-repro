...
// Component: Utils/src/CyclicNTT.cpp
// Purpose: Cyclic NTT preprocessing for packed block-circulant weights.
// Maintenance: DAZG-Orbit integration, acceleration, and reproducibility layer.
// Provenance and licenses: see NOTICE_UPSTREAM.md and LICENSE_NOTICE.md.
// Validated scope: frozen N=10 and checkpoint-013 balanced N=100.
// Security boundary: reveal correctness backend; security_claim=0.

// DAZG-Orbit cyclic-NTT acceleration:
// block-circulant plaintext preprocessing uses an O(n log n) cyclic transform
// and preserves coefficient/ring order expected by the packed HE operators.
...
uint64_t CyclicNTT::FindPrimitiveRoot() {
    // DAZG_ORBIT_V539_NTT_SIZE1_IDENTITY_FIX
    // n=1 is the identity DFT. The old code checked omega^(n/2),
    // which becomes omega^0 and rejects every candidate. That made
    // final-FC block_size=1 fail before inference with:
    //   Could not find primitive n-th root of unity
    if (n_ == 1) {
        const char* quiet = std::getenv("DAZG_ORBIT_V539_NTT1_QUIET");
        if (quiet == nullptr || quiet[0] == '\0' || quiet[0] == '0') {
            std::cerr << "[DAZG_ORBIT_V539_NTT_SIZE1_IDENTITY_FIX] n=1 omega=1" << std::endl;
        }
        return 1;
    }
